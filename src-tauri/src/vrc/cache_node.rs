...
impl CacheNode {
    ...
    pub fn latest_interp(&self) -> f32 {
        ...
        let velocity = self.value_delta(latest_value, &old_value) / latest_time.duration_since(*old_time).unwrap().as_secs_f32();
        let seconds_since_last = SystemTime::now().duration_since(*latest_time).unwrap().as_secs_f32();
    ...
    pub fn velocity_since(&self, limit: &SystemTime) -> f32 {
        ...
        for (index, (val, time)) in self.values.iter().enumerate() {
            if *time > *limit { // Shouldn't be necessary?
    ...
    pub fn velocity_by_entry(&self, entries_back: usize) -> Result<f32, RetrievalError> {
        ...
        if let Some((val, time)) = self.values.back() {
            if let Some((val_late, time_late)) = self.values.get(entries_back) {
                ...
                return Err(RetrievalError::CacheTooSmall(self.values.len(), entries_back))
            }
        } else {
    ...
    pub fn update(&mut self, value: OscType) -> Result<(), WrongNodeTypeError> {
        if discriminant(&self.osc_type) != discriminant(&value) {
            return Err(WrongNodeTypeError{entered: value, expected: self.osc_type.clone()});
        }
    ...
    fn value_delta(&self, value: &OscType, val_late: &OscType) -> f32 {
        match value {
            ...
            OscType::Bool(bool) => if *bool == val_late.clone().bool().unwrap() { 0.0 } else { 1.0 },
            _ => unimplemented!(),
        }
    }
}
...
pub struct WrongNodeTypeError{entered: OscType, expected: OscType}
