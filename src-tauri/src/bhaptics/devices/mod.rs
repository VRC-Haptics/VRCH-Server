...
pub fn start_bt_nonblocking(device_list: Arc<Mutex<Vec<Device>>>) -> Result<(), tauri_plugin_blec::Error> {
...
async fn bt_scan_and_connect_loop(device_list: Arc<Mutex<Vec<Device>>>) -> Result<(), tauri_plugin_blec::Error> {
            ...
            let connection_tasks = devices
                ...
                .map(|device| {
                    ...
                    tokio::spawn(async move {
                        connect_to_device(device, device_list_clone).await
                    })
                })
                .collect::<Vec<_>>();
